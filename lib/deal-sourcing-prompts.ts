...
{ "industry_evaluation": { "score": 0, "startup_industry": "string" }, "stage_evaluation": { "score": 0, "stage": "string" }, "funding_evaluation": { "score": 0, "funding": "string" }, "overall_thesis_alignment_reasoning": "string", "auto_reject_flag": boolean }`;
...
1. parsed_startup_data: (Phase 1 JSON — at minimum **problem**, **market**, **company_overview**, **fundraising** so searches can use industry, persona, and market claims together).
2. company_name: "[Company]"
...
1. parsed_startup_data: (Phase 1 JSON — at minimum **solution**, **market**, **company_overview**, **notable_claims** for technology and differentiation context).
2. company_name: "[Company]"
...
3. \`core_signal_scores\` — structured object (NOT only numeric scores):
   - **founder**: collective team scores from Founder Prompt B + \`signal_completeness\` (LOW|MEDIUM|HIGH).
   - **traction**: traction strength / growth / stage-adjusted scores + \`signal_completeness\`.
   - **problem**: Phase 3C \`scores\` + **signal_interpretation** (includes signal_completeness from Problem agent).
   - **solution**: Phase 3D \`scores\` + **signal_interpretation** (includes signal_completeness from Solution agent).
   - **market**: Phase 1 \`market\` claims (TAM/SAM/SOM etc.); there is no separate Phase 3 Market agent in this pipeline—use these claims + Problem/Solution outputs to reason about market risk.
...
3. **Fragility Mapping**: Use **signal_completeness** and **signal_interpretation** from Phase 3 Problem & Solution JSONs (and founder/traction completeness) to locate the weakest part of the chain—do not rely on scores alone.
