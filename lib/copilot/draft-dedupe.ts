 ...
 * Keep in sync with extension/shared/draft-dedupe.ts.
 */
...
/** |A∩B| / min(|A|,|B|) — catches when the shorter snippet is almost entirely explained by the longer one. */
...
/**
 * True if `incoming` says substantially the same thing as any existing string (Wikipedia-style rewrites).
 */
