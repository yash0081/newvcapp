...
export function CompanyDocuments(props: { dealId: string; initialDocs: DocRow[] }) {
  ...
  return (
    <div className="w-full space-y-4">
      <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-3">
        ...
        <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-zinc-950">Upload PDF</p>
            </div>
      ...
      {err && <div className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{err}</div>}

      <div className="grid w-full gap-4 lg:grid-cols-[280px_1fr]">
        <div className="rounded-2xl border border-zinc-200 bg-white p-3">
          <div className="mb-2 flex items-center gap-2">
            <Folder className="h-4 w-4 text-zinc-500" />
            <p className="text-sm font-semibold text-zinc-950">Folders</p>
          </div>
          <button
            type="button"
        ...
        <div className="min-w-0 space-y-2">
          {visibleDocs.length === 0 ? (
            ...
            visibleDocs.map((d) => (
              <div
                ...
                className="flex w-full min-w-0 items-center gap-3 rounded-xl border border-zinc-200 bg-white px-3 py-2 transition-colors hover:bg-zinc-50"
              >
                <GripVertical className="h-4 w-4 shrink-0 text-zinc-300" />
                <FileText className="h-4 w-4 shrink-0 text-zinc-500" />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-medium text-zinc-950">{d.original_filename || d.id}</div>
                  <div className="mt-0.5 flex items-center gap-2 text-xs text-zinc-500">
                    <span className="inline-flex items-center rounded-xl border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[11px] font-medium">
                      {displayStatus(d.status)}
                    </span>
                    <span>{new Date(d.created_at).toLocaleString()}</span>
                  </div>
                </div>
                <button onClick={() => openDocument(d.id, d)} className="rounded-xl border border-zinc-200 px-2 py-1 text-xs font-medium text-zinc-700 hover:bg-white">
                  Open
                </button>
                <button
                  disabled={busy != null || isPending}
                  onClick={() => deleteDoc(d.id)}
                  className="rounded-xl border border-rose-200 bg-rose-50 px-2 py-1 text-xs font-medium text-rose-700 hover:bg-rose-100 disabled:opacity-50"
                >
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-3">
            <p className="mb-2 text-xs text-zinc-500">Drop documents here to move to <span className="font-medium text-zinc-700">{activeFolder}</span></p>
            <div
              ...
              className="rounded-xl border border-dashed border-zinc-200 bg-zinc-50 p-5 text-center text-sm text-zinc-500"
            >
